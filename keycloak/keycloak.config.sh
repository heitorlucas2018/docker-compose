...
KEYCLOAK_PROXY_CLIENT_SECRET=$(cat /secrets/KEYCLOAK_PROXY)
KEYCLOAK_PROXY_USER=${KEYCLOAK_PROXY_USER:-"user-proxy"}
KEYCLOAK_PROXY_USER_SECRET=$(cat /secrets/KEYCLOAK_PROXY_USER)
...
createRealm() {
  ...
  curl -X POST "${KEYCLOAK_HOST}/admin/realms" \
    ...
    -d "$(cat /keycloak/customRealmRepresentation.json  | sed 's/$KEYCLOAK_CREATE_REALM/teste/g')"
